...
public class LoaderInfo implements Serializable {
    ...
    private static final String HOWLLOADER_CSV_FORMAT_REGEX = ".*,.*";
    private static final char HOWLLOADER_CSV_SEPARATOR_CHAR = ',';
    ...
    /**
     * Copy Constructor for LoaderInfo
     * @param o
     */
    public LoaderInfo(LoaderInfo o){
     ...
     */
    public LoaderInfo(String infoString){
        //Currently only storage driver class name, the input would have to be parsed as more info is added
        if (infoString != null){
            if (! infoString.matches(HOWLLOADER_CSV_FORMAT_REGEX)){
                this.setInputDriverClass(infoString);
            } else {
                int commaPosn = infoString.indexOf(HOWLLOADER_CSV_SEPARATOR_CHAR);
     ...
     */
    public LoaderInfo(String inputDriverClass, String inputDriverArgs) {
    ...
    public boolean equals(Object obj) {
        ...
        LoaderInfo other = (LoaderInfo) obj;
